...
test.describe('Navigation links test', () => {
    test.beforeEach(async ({ page }) => {
      // Navigate to the "Architecture" category page before each test
      await page.goto('https://relay.prototyp.digital/category/architecture');
...
test('"Architecture" link returns to the section', async ({ page }) => {
    await page.locator('article img').first().click();
...
test('Category link in the card description works', async ({ page }) => {
    // Locate all links within articles that link to Architecture 
    const categoryLinks = page.locator('article a[href*="/category/architecture/"]');
    const count = await categoryLinks.count();
    // Click each category link and verify it navigates to the Architecture category page, then go back
...
test('Only "Architecture" articles are listed', async ({ page }) => {
    const categoryLabels = page.locator('article p');
    const count = await categoryLabels.count();
    // Check each article description contains the text "Architecture", ensuring no other category articles appear
    for (let i = 0; i < count; i++) {
        await expect(categoryLabels.nth(i)).toContainText('Architecture');
    }
  })
