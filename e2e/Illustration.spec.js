...
test.describe('Navigation links test', () => {
    test.beforeEach(async ({ page }) => {
        // Navigate to the "Illustration" category page before each test
      await page.goto('https://relay.prototyp.digital/category/illustration');
...
test('Category link in the card description works', async ({ page }) => {
    // Locate all links within articles that link to the Illustration
    const categoryLinks = page.locator('article a[href*="/category/illustration/"]');
    const count = await categoryLinks.count();
    // Click each category link and verify it navigates to the Illustration category page, then go back
...
test('Only "Illustration" articles are listed', async ({ page }) => {
    const categoryLabels = page.locator('article p');
    const count = await categoryLabels.count();
    // Check each article description contains the text "Illustration", ensuring no other category articles appear
    for (let i = 0; i < count; i++) {
        await expect(categoryLabels.nth(i)).toContainText('Illustration');
    }
  })
